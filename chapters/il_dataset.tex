...
Questa sezione è divisa in due parti: nella prima verranno illustrate alcune tra le principali tecniche di \textit{Digital Image Preprocessing}, esponendo i dettagli matematici ed esplorando le loro applicazioni; nella seconda si spiegherà quali di queste tecniche, in che ordine e per quali motivi sono state utilizzate.
...
Spesso questi algoritmi hanno precise formulazioni matematiche perché ogni immagine può essere pensata come una matrice bidimensionale, se in scala di grigi, oppure tridimensionale se a colori.
Un'immagine in scala di grigi è rappresentata da una matrice bidimensionale in cui ogni elemento appartiene all'intervallo $[0,255]$, $0$ corrisponde al colore nero mentre $255$ corrisponde al bianco.
...
Nell'ambito del \textit{Image Preprocessing} con convoluzione si intende l'operazione che permette di effettuare una somma pesata tra un pixel di un'immagine e gli elementi a lui vicini.
I pesi sono definiti in una matrice, detta \textit{kernel} o filtro, di dimensioni non superiori a quelle dell'immagine.
Solitamente i kernel hanno dimensione $3x3$ o $5x5$.
Sfrutteremo ora un esempio per spiegare come viene effettuata una convoluzione, poi verrà illustrato uno pseudocodice generale.
In Figura~\ref{fig:conv_example} sono illustrate, da sinistra a destra: l'immagine di partenza $I$, il filtro $K$ e l'immagine risultato $O$.
Il simbolo $*$ denota l'operazione di convoluzione e non è da confondere con il prodotto elemento per elemento.
...
\begin{itemize}
  ...
  \item I valori così ottenuti dovranno essere sommati.
    Effettuando i conti si osserva che il risultato combacia con il valore in alto a sinistra dell'immagine risultato.
  \item Il prossimo passo consiste nel far traslare il kernel di una cella a sinistra, calcolare la somma di prodotti rispetto ai nuovi valori ed infine salvare il risultato in $O$ una cella a destra rispetto a prima.
  \item Si prosegue in questo modo fino a che l'ultima colonna di $K$ non combacia con l'ultima colonna di $I$, questo coincide con il riempimento della prima riga in $O$.
  \item Ora bisogna posizionare $K$ in modo che l'elemento nell'angolo superiore sinistro sia sopra all'elemento in $I$ sulla prima colonna della seconda riga.
    Si effettuano prodotti e somme, salvando il risultato nella prima cella libera della seconda riga in $O$.
...
Se si vuole ottenere un'immagine 
...
% Una convoluzione tra un'immagine e un kernel viene effettuata facendo scorrere il kernel sull'immagine in modo che il kernel venga centrato almeno una volta su ogni elemento dell'immagine tale che il kernel non sbordi dall'immagine.
%Una convoluzione tra un'immagine $I$ ed un kernel $K$ può essere descritta con il seguente pseudocodice.
%
%\begin{lstlisting}
%output_image = new empty image 
%
...
% DECIDERE! 
% Opzione 2: Prima spiego gli algoritmi poi summary dell'ordine e dei motivi per cui sono stai applicati.

Descrizione sommaria di ciò che viene fatto alle immagini ed in che ordine.

Prima verranno introdotti algoritmi che sono alla base o fanno parte di quelli che affronteremo poi.
...
\subsection {Masking}
Basta dire che è una operazione di AND tra mask e immagine.
